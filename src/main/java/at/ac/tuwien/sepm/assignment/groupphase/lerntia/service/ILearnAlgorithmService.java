...
/**
 * The LearnAlgorithmService Class will handle the mayority of calculation and value-management of the learn algorithm.
...
public interface ILearnAlgorithmService {
     ...
     * (subtract failurevalue from succesvalue or only 1 point if the failurevalue is higher than the successvalue)
     *
    ...
    /**
     * Update the Failure quota (between 0 and 10) of an existing Question. After an Question has been falsely answered the quota of it
     * appearing again will be rised in order help the User learn this Question better. If the failure rate is higher than the success rate
     ...
     * (subtract succesvalue from failurevalue or only 1 negative point if the succesvalue is higher than the succesvalue.)
     *
    ...
    /**
     * Prepare the Values of a Questionaire for the Algorithm and sort the questions so that the questions with the lowest values start first
     *
    ...
    /**
     * Change the Values of Questions in the Database when another questionnaire has been selected or the programm shuts down.
     *
    ...
    /**
     * Updates the values of the current questionnaire so those values dont get lost
     *
     * @throws ServiceException if the method can't update those values and cant send them to the DAO layer.
     * */
